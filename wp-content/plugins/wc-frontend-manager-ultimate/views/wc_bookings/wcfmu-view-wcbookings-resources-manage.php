...
if( wcfm_is_vendor() ) {
	$is_resource_for_vendor = $WCFM->wcfm_vendor_support->wcfm_is_component_for_vendor( $resource_id, 'resource' );
	if( !$is_resource_for_vendor ) {
		if( apply_filters( 'wcfm_is_show_resource_restrict_message', true, $resource_id ) ) {
			wcfm_restriction_message_show( "Restricted Resource" );
		} else {
			echo apply_filters( 'wcfm_show_custom_resource_restrict_message', '', $resource_id );
		}
		return;
	}
}
