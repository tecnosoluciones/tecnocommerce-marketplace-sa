 ...
 */
if(!function_exists('conditional_badges_modes')) {
	function get_conditional_badges_modes() {
		$conditional_badges_modes = array(
			''       			=> __( '- Select -', 'wc-frontend-manager-ultimate' ),
			'sales'       		=> sprintf( __( 'Total Sales (%s)', 'wc-frontend-manager-ultimate' ), get_woocommerce_currency_symbol() ),
			'orders'         	=> __( 'Total Orders Completed', 'wc-frontend-manager-ultimate' ),
			'registration_time' => __( 'User Registration Time (days)', 'wc-frontend-manager-ultimate' ),
		);
		
		return apply_filters( 'wcfm_conditional_badges_modes', $conditional_badges_modes );
	}
}
?>
