...
class WCFMu_Vendor_Badges {
	...
	public function __construct() {
		...
		//Conditional badges
		add_action( 'after_wcfm_vendor_badges', array( &$this, 'show_conditional_badges' ), 10, 2 );
	}
	
	function wcfmu_vendor_badges_settings( $wcfm_options ) {
		global $WCFM, $WCFMu;
		$wcfm_vendor_badges_options = get_option( 'wcfm_vendor_badges_options', array() );
		$wcfm_conditional_badges_rules = get_option( 'wcfm_conditional_badges_rules', array() );
		?>
		<!-- collapsible -->
		...
		<div class="wcfm-container">
			<div id="wcfm_settings_form_vendor_badges_expander" class="wcfm-content">
				...
				<?php
				$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_conditional_badges_settings_fields', array(
					"wcfm_conditional_badges_rules" 	=> array(
	...
	function show_conditional_badges( $vendor_id, $badge_classses ) {
		...
		if( !empty( $wcfm_conditional_badges_rules ) ) {
			foreach( $wcfm_conditional_badges_rules as $rule ) {
				...
				if( !isset( $rule['is_active'], $rule['mode'], $rule['rule'], $rule['value'] ) ) continue;
