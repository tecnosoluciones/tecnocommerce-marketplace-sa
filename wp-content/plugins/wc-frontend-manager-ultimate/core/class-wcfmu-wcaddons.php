...
class WCFMu_WCAddons {
	  	...
	  	switch( $end_point ) {
	  		case 'wcfm-products-manage':
				...
				$params = array(
					...
					'i18n'     => array(
						'required_fields'       	=> __( 'All fields must have a title and/or option name. Please review the settings highlighted in red border.', 'wc-frontend-manager-ultimate' ),
						'limit_price_range'         => __( 'Limit price range', 'wc-frontend-manager-ultimate' ),
						'limit_quantity_range'      => __( 'Limit quantity range', 'wc-frontend-manager-ultimate' ),
						'limit_character_length'    => __( 'Limit character length', 'wc-frontend-manager-ultimate' ),
						'restrictions'              => __( 'Restrictions', 'wc-frontend-manager-ultimate' ),
						'confirm_remove_addon'      => __( 'Are you sure you want remove this add-on field?', 'wc-frontend-manager-ultimate' ),
						'confirm_remove_option'     => __( 'Are you sure you want delete this option?', 'wc-frontend-manager-ultimate' ),
						'add_image_swatch'          => __( 'Add Image Swatch', 'wc-frontend-manager-ultimate' ),
						'add_image'                 => __( 'Add Image', 'wc-frontend-manager-ultimate' ),
					),
				);
	...
	public function convert_type_name( $type = '' ) {
		switch ( $type ) {
			case 'checkboxes':
				$name = __( 'Checkbox', 'wc-frontend-manager-ultimate' );
				break;
			case 'custom_price':
				$name = __( 'Price', 'wc-frontend-manager-ultimate' );
				break;
			case 'input_multiplier':
				$name = __( 'Quantity', 'wc-frontend-manager-ultimate' );
				break;
			case 'custom_text':
				$name = __( 'Short Text', 'wc-frontend-manager-ultimate' );
				break;
			case 'custom_textarea':
				$name = __( 'Long Text', 'wc-frontend-manager-ultimate' );
				break;
			case 'file_upload':
				$name = __( 'File Upload', 'wc-frontend-manager-ultimate' );
				break;
			case 'select':
				$name = __( 'Dropdown', 'wc-frontend-manager-ultimate' );
				break;
			case 'multiple_choice':
			default:
				$name = __( 'Multiple Choice', 'wc-frontend-manager-ultimate' );
				break;
		}
	 ...
	 */
  	function wcaddons_wcfm_products_manage( $product_id ) {
		...
		$group_types = apply_filters( 'wcfm_wcaddon_group_types', array(  
			'custom_price'               => __( 'Additional custom price input', 'wc-frontend-manager-ultimate' ),
			...
			<div id="wcfm_products_manage_form_wcaddons_expander" class="wcfm-content">
				...
				$WCFM->wcfm_fields->wcfm_generate_form_field( apply_filters( 'wcfm_product_manage_fields_wcaddons', array( 
					"_product_addons" => array(
						...
						'type' => 'multiinput', 'class' => 'wcfm_ele wcaddons', 
						'label_class' => 'wcfm_title wcaddons', 
						'value' => $_product_addons, 
						'options' => array(
							...
							"options" => array(
								'label' => __('Options', 'wc-frontend-manager-ultimate') . '<span class="fields_collapser wcfmfa fa-arrow-circle-down"></span>', 
								'type' => 'multiinput', 
								'class' => 'wcfm_ele wcaddons wcfm_wcaddons_fields', 
								'label_class' => 'wcfm_title wcaddons', 
								'options' => array(
