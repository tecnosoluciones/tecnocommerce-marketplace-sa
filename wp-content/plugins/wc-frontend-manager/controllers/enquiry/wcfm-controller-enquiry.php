      ...
      return $wcfm_enquirys_array;
		...
		if(!empty($wcfm_enquirys_array)) {
			...
			foreach($wcfm_enquirys_array as $wcfm_enquirys_single) {
				// Enquiry
				$wcfm_enquirys_json_arr[$index][] =  '<a href="' . esc_url(get_wcfm_enquiry_manage_url($wcfm_enquirys_single->ID)) . '" class="wcfm_dashboard_item_title">' . apply_filters( 'wcfm_enquiry_message_display', $wcfm_enquirys_single->enquiry, $wcfm_enquirys_single->ID ) . '</a>';
				
				// Product
				if( $wcfm_enquirys_single->product_id ) {
					$wcfm_enquirys_json_arr[$index][] =  '<a class="wcfm-enquiry-product" target="_blank" href="' . esc_url(get_permalink($wcfm_enquirys_single->product_id)) . '">' . get_the_title($wcfm_enquirys_single->product_id) . '</a>';
				} else {
