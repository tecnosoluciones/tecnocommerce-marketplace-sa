...
require_once dirname( __FILE__ ) . '/class-wcfm-gateway-direct-paypal-response.php';

/**
 * WC_Gateway_Paypal_IPN_Handler class.
 */
class WCFM_PG_Direct_Paypal_IPN_Handler extends WCFM_PG_Direct_Paypal_Response {
	...
	public function __construct() {
		add_action( 'woocommerce_api_wc_gateway_paypal', array( $this, 'check_response' ), 9 );
		add_action( 'wcfm-valid-paypal-standard-ipn-request', array( $this, 'valid_response' ) );

		$paypal_setting = get_option( 'woocommerce_paypal_settings', array() );
		$testmode       = isset( $paypal_setting['testmode'] ) ? $paypal_setting['testmode'] : 'no';
		$this->sandbox  = 'yes' === $testmode;
	}
				...
				if( wcfm_is_vendor( $vendor_id ) ) {
					...
					if( $paypal_email ) {
						if ( ! empty( $_POST ) && $this->validate_ipn() ) { // WPCS: CSRF ok.
							WC_Gateway_Paypal::log( "Marketplace Direct PayPal IPN Check" );
							
							$this->receiver_email = $paypal_email;
							
							// phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
							do_action( 'wcfm-valid-paypal-standard-ipn-request', $posted );
							exit;
						}
	...
	protected function payment_status_completed( $order, $posted ) {
		...
		if ( 'completed' === $posted['payment_status'] ) {
			...
			$this->payment_complete( $order, ( ! empty( $posted['txn_id'] ) ? wc_clean( $posted['txn_id'] ) : '' ), __( 'Marketplace Direct PayPal IPN payment completed', 'woocommerce' ) );
		} else {
